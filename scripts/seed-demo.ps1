...
$issues = @(
    @{number=1;  title="Critical auth bypass in OAuth flow"; body="Security vulnerability in the OAuth callback handler."; labels=@(@{name="priority:critical"},@{name="security"},@{name="bug"}); assignees=@(@{login="roryp"},@{login="alice"},@{login="bob"}); createdAt=$ts.twoDaysAgo; updatedAt=$ts.recent; state="open"}
    ...
    @{number=3;  title="Fix typo in README"; body="Small typo fix needed"; labels=@(@{name="good-first-issue"},@{name="quick-win"}); assignees=@(@{login="bob"}); createdAt=$ts.yesterday; updatedAt=$ts.recent3; state="open"}
    ...
    @{number=5;  title="URGENT: Production memory leak"; body=""; labels=@(@{name="urgent"},@{name="bug"},@{name="priority:high"}); assignees=@(); createdAt=$ts.twoWeeksAgo; updatedAt=$ts.afterHours; state="open"}
    @{number=6;  title="URGENT: API rate limiting broken"; body=""; labels=@(@{name="urgent"},@{name="bug"}); assignees=@(); createdAt=$ts.twoDaysAgo; updatedAt=$ts.afterHours; state="open"}
    @{number=7;  title="URGENT: Database connection pool exhaustion"; body="Pool runs out under load."; labels=@(@{name="urgent"},@{name="priority:critical"}); assignees=@(@{login="alice"}); createdAt=$ts.monthAgo; updatedAt=$ts.recent; state="open"}
    ...
    @{number=9;  title="Something unclear"; body=""; labels=@(); assignees=@(); createdAt=$ts.twoMonthsAgo; updatedAt=$ts.monthAgo; state="open"}
    @{number=10; title="Another vague issue"; body=""; labels=@(); assignees=@(); createdAt=$ts.monthAgo; updatedAt=$ts.twoWeeksAgo; state="open"}
    @{number=11; title="CI pipeline failing intermittently"; body="GitHub Actions fails randomly on test step."; labels=@(@{name="ci"},@{name="devops"},@{name="chore"}); assignees=@(@{login="bob"}); createdAt=$ts.weekAgo; updatedAt=$ts.recent3; state="open"}
    @{number=12; title="Write API documentation"; body="Need OpenAPI specs for all endpoints."; labels=@(@{name="documentation"},@{name="tech-debt"}); assignees=@(@{login="alice"}); createdAt=$ts.twoWeeksAgo; updatedAt=$ts.weekAgo; state="open"}
    ...
    @{number=14; title="Fix CORS headers on demo endpoints"; body="Quick config change needed"; labels=@(@{name="quick-win"},@{name="bug"},@{name="size:s"}); assignees=@(@{login="bob"}); createdAt=$ts.yesterday; updatedAt=$ts.recent; state="open"}
    @{number=15; title="Stale tracking issue from last quarter"; body=""; labels=@(@{name="triage"}); assignees=@(); createdAt=$ts.twoMonthsAgo; updatedAt=$ts.twoMonthsAgo; state="open"}
